...
export default function ManualBitacoraModal({ isOpen, onClose }) {
  ...
  const typesList = [
    ...
    { name: "PAGO", color: "border-violet-500", desc: "Conciliación bancaria." },
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-gray-50 rounded-2xl shadow-2xl border border-white/20">
        ...
        <div className="sticky top-0 z-10 bg-white border-b px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Manual de Usuario - Panel de Control</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 space-y-8">
          ...
          <section className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="space-y-3">
              {statusList.map((st) => (
                <div key={st.name} className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-xl bg-gray-50 border border-gray-100 hover:bg-white hover:border-blue-100 transition-colors">
                  <span className={`w-40 text-center px-3 py-1.5 rounded-full text-[10px] font-black tracking-wider ${st.color}`}>
                    {st.name}
                  </span>
          ...
          <section className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {typesList.map((type) => (
                <div key={type.name} className={`p-4 bg-white border-l-4 rounded-r-xl shadow-sm hover:shadow-md transition-shadow ${type.color}`}>
                  <h4 className="font-bold text-xs tracking-widest text-gray-800 mb-1">{type.name}</h4>
          ...
          {/* Section: Rule of Gold */}
          <section className="bg-orange-50 rounded-xl p-6 border border-orange-100 flex gap-4">
            <PencilSquareIcon className="h-6 w-6 text-orange-600 flex-shrink-0 mt-1" />
            <div>
              <h3 className="font-bold text-orange-900 mb-2">Regla de Oro: Vinculación</h3>
              <p className="text-sm text-orange-800 leading-relaxed">
                Para que el panel identifique el correo automáticamente, es <strong>obligatorio</strong> incluir el código <strong>[RefID: ***]</strong> en el cuerpo del correo al responder a los interesados.
              </p>
            </div>
          </section>
        </div>
