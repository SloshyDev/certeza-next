...
export default function ExtractorModal({ isOpen, onClose, onSuccess }) {
  ...
  // Cargar plantillas, asesores y gerentes al abrir el modal
  useEffect(() => {
    if (isOpen) {
      loadTemplates();
      loadAsesores();
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden">
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white px-6 py-4 flex items-center justify-between sticky top-0 z-10">
            <div>
              <h2 className="text-2xl font-bold">📄 Lector de Pólizas</h2>
              ...
              <div className="space-y-6">
                        ...
                        {field === "quincena" ? (
                          <div className="flex gap-2">
                            ...
                            <input
                              ...
                              className="w-24 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition text-sm"
                              placeholder="Año"
                              min="2020"
                              max="2099"
                            />
                ...
                <div className="flex gap-4 pt-4 border-t">
                  <button
                    onClick={handleSave}
                    disabled={loading}
                    className="flex-1 bg-gradient-to-r from-green-600 to-green-700 text-white font-bold py-3 px-6 rounded-lg hover:from-green-700 hover:to-green-800 disabled:opacity-50 transition-all duration-200"
                  >
                    {loading ? "Guardando..." : "💾 Guardar Datos"}
                  </button>
                  <button
                    onClick={handleReset}
                    className="flex-1 bg-gradient-to-r from-gray-500 to-gray-600 text-white font-bold py-3 px-6 rounded-lg hover:from-gray-600 hover:to-gray-700 transition-all duration-200"
                  >
                    🔄 Nueva Extracción
                  </button>
                </div>
