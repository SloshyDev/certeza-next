...
export default async function Home(props) {
  ...
  let start = normalize(rawStart);
  let end = normalize(rawEnd);
  if (start > end) {
    const tmp = start;
    start = end;
    end = tmp;
  ...
  const rawTipos = searchParams && searchParams.tipo;
    ...
    <div className="py-6">
      <main className="container-responsive px-4 sm:px-6">
        ...
        {session ? (
              ...
              >
                <div className="flex flex-col gap-1 w-full sm:w-auto">
                  ...
                  <input
                    ...
                    type="date"
                    defaultValue={start}
                    className="h-10 rounded-lg border border-border bg-white/10 backdrop-blur-md px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent"
                  />
                </div>
                <div className="flex flex-col gap-1 w-full sm:w-auto">
                ...
                <div className="flex flex-col gap-1 w-full sm:w-auto">
                  <label className="text-sm opacity-0 select-none">
                    Aplicar
                  </label>
          ...
          >
            <button className="btn-primary">Entrar con Microsoft</button>
          </form>
        )}
      </main>
    </div>
